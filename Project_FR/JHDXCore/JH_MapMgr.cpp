...
#include"JH_ObjMgr.h"
	...
	//	ibuf[0] = pMap->m_VertexData.size();
	//	fwrite((void*)ibuf, sizeof(int), 1, fp);
	//	int layer = 0;
	//	for (int iVertex = 0; iVertex < pMap->m_VertexData.size(); iVertex++)
	//	{

	//		fbuf[0] = pMap->m_VertexData[iVertex].p.x; fbuf[1] = pMap->m_VertexData[iVertex].p.y; fbuf[2] = pMap->m_VertexData[iVertex].p.z;
	//		fbuf[3] = pMap->m_VertexData[iVertex].n.x; fbuf[4] = pMap->m_VertexData[iVertex].n.y; fbuf[5] = pMap->m_VertexData[iVertex].n.z;
	//		fbuf[6] = pMap->m_VertexData[iVertex].c.x; fbuf[7] = pMap->m_VertexData[iVertex].c.y; fbuf[8] = pMap->m_VertexData[iVertex].c.z; fbuf[9] = pMap->m_VertexData[iVertex].c.w;
	//		fbuf[10] = pMap->m_VertexData[iVertex].t.x; fbuf[11] = pMap->m_VertexData[iVertex].t.y;
	...
	//	//Map Basic Text Data
	//
	//	std::shared_ptr<LEVEL> GMData;
	//	GMData = std::make_shared<LEVEL>();
	...
	//	m_MapData.m_VertexData.resize(ibuf[0]);
	//	int layer = 0;
	//	for (int iVertex = 0; iVertex < m_MapData.m_VertexData.size(); iVertex++)
	//	{
	//		fread((void*)fbuf, sizeof(float), 12, fp);
	//		m_MapData.m_VertexData[iVertex].p.x = fbuf[0];  m_MapData.m_VertexData[iVertex].p.y = fbuf[1];  m_MapData.m_VertexData[iVertex].p.z = fbuf[2];
	//		m_MapData.m_VertexData[iVertex].n.x = fbuf[3];  m_MapData.m_VertexData[iVertex].n.y = fbuf[4]; m_MapData.m_VertexData[iVertex].n.z = fbuf[5];
	//		m_MapData.m_VertexData[iVertex].c.x = fbuf[6];  m_MapData.m_VertexData[iVertex].c.y = fbuf[7]; m_MapData.m_VertexData[iVertex].c.z = fbuf[8]; m_MapData.m_VertexData[iVertex].c.w = fbuf[9];
	//		m_MapData.m_VertexData[iVertex].t.x = fbuf[10]; m_MapData.m_VertexData[iVertex].t.y = fbuf[11];
	...
	//	//SAFE_NEW(GMData->m_pQuadTree->m_pRootNode, JH_Node);
	...
	JH_Map* JH_MapMgr::CreateMap(std::wstring MapName,int iWidth,
		int iHeight,
		int iCellCount,
		int iCellSize,
	...
	//	pMap->m_VertexData.resize(m_MapData.m_VertexData.size());
	//	std::copy(m_MapData.m_VertexData.begin(), m_MapData.m_VertexData.end(), pMap->m_VertexData.begin());
	...
	void JH_MapMgr::SaveToQuadTree(JH_Node* pNode, FILE* fp)
	{
	...
	void JH_MapMgr::LoadToQuadTree(JH_Node* pNode, FILE* fp)
	{
		if (pNode == nullptr)return;
