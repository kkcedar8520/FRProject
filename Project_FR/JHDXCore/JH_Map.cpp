...
{//--------------------------------------------------------------------------------------	
	...
	m_pTangentVB.Attach(DX::CreateVertexBuffer(m_dxHelper.m_pd3dDevice.Get(), &m_TangentList.at(0), m_TangentList.size(), sizeof(D3DXVECTOR3)));
...
{
	HRESULT hr = S_OK;
	if (m_dxHelper.m_pSRV = DX::CreateShaderResourceView(m_dxHelper.m_pd3dDevice.Get(), pszTexFileName))
	
	m_iTexNum=I_Texture.Add(m_dxHelper.m_pd3dDevice.Get(), m_pNormMapFileName);
