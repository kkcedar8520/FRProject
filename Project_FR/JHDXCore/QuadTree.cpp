...
void HQuadTree::CreateBB(JH_Node* pNode)
{
	...
	// 노말 방향 안쪽으로
	pNode->m_BoxPlane[0].CreatePlane(BoxVertex[5], BoxVertex[4], BoxVertex[1]);	// 좌 평면(left)
	pNode->m_BoxPlane[1].CreatePlane(BoxVertex[3], BoxVertex[7], BoxVertex[2]);	// 우 평면(right)
	...
	pNode->m_BoxPlane[2].CreatePlane(BoxVertex[0], BoxVertex[3], BoxVertex[1]);	// 근 평면(near)
	pNode->m_BoxPlane[3].CreatePlane(BoxVertex[6], BoxVertex[7], BoxVertex[5]);	// 원 평면(far)
...
JH_Obj* HQuadTree::GetSelectObj()
{

	//통과하는 모든노드찾기
	GetSelectNode(m_pRootNode);

	if (m_SelectNodeList.size() <= 0) { return nullptr; }


	JH_Obj* pObj = nullptr;
	//찾은노드들이 가지고있는 오브젝트들중에서 레이와의 거리가 가장짧은것이 선택한 오브젝트다
	for (auto Node : m_SelectNodeList)
	{
...
bool HQuadTree::ObjectAddNode(JH_Obj* Obj)
{
	...
	if (pos == P_FRONT)
	{
		...
		if (pNode)
		{
			//오브젝트의 높이가 노드의 높이보다 크다면 노드의박스 높이를 오브젝트 높이만큼 맞춰줌
			CreateBB(pNode);
...
bool HQuadTree::ChangeObjectNode(JH_Obj* Obj)
{
	bool b = false;
	//if (!m_pRootNode) return false;
	//JH_Node* pNode = nullptr;
...
{
	//큰노드들부터 순서대로 자식노드가 더이상없으면 반복을 종료
