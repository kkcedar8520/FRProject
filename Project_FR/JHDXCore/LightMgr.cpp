#pragma once
#include "LightMgr.h"
/*void LightMgr::SetCamera(JHCamera* pCamera)
{
	m_pCamera = pCamera;
}*/
void LightMgr::SetDevice(ID3D11Device* pdevcie)
{
	m_pDevice = pdevcie;
}
void LightMgr::SetContext(ID3D11DeviceContext* pContext)
{
	m_pContext = pContext;
}
...
bool LightMgr::Init()
{
	m_pLightConstantBuffer.resize(CONSTANT_BUFFER_SIZE);
	m_pLightConstantBuffer[0].Attach(DX::MakeConstantBuffer(DX::GetDevice().Get(), &m_cbLight, sizeof(LightConstantBuffer), 1));
...
bool LightMgr::Frame()
{
	for (int i = 0; i < m_LightObjList.size(); i++)
	{
